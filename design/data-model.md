# WestMatch Data Model

## Schema Definition

```typescript
import { defineSchema, defineTable } from "convex/server";
import { v } from "convex/values";

export default defineSchema({
  profiles: defineTable({
    userId: v.string(),
    displayName: v.string(), // What users want to be called
    gender: v.union(v.literal("male"), v.literal("female")),
    interestedIn: v.union(v.literal("male"), v.literal("female")),
    birthDate: v.number(),
    year: v.union(
      v.literal("freshman"),
      v.literal("sophomore"),
      v.literal("junior"),
      v.literal("senior")
    ),
    preferredYears: v.optional(
      v.object({
        freshman: v.boolean(),
        sophomore: v.boolean(),
        junior: v.boolean(),
        senior: v.boolean(),
      })
    ),
    major: v.string(),
    bio: v.optional(v.string()),
    drinking: v.optional(v.string()),
    smoking: v.optional(v.string()),
    profileComplete: v.boolean(),
    updatedAt: v.number(),
  })
    .index("by_userId", ["userId"])
    .index("by_interestedIn_year", ["interestedIn", "year"]),

  profilePhotos: defineTable({
    id: v.string(),
    profileId: v.id("profiles"),
    key: v.string(),
    orderIndex: v.number(),
    deletedAt: v.optional(v.number()),
  })
    .index("by_profileId_orderIndex", ["profileId", "orderIndex"])
    .index("by_id", ["id"]),

  profilePrompts: defineTable({
    id: v.string(),
    profileId: v.id("profiles"),
    promptId: v.string(),
    answer: v.string(),
    orderIndex: v.number(),
    deletedAt: v.optional(v.number()),
  })
    .index("by_profileId_orderIndex", ["profileId", "orderIndex"])
    .index("by_id", ["id"]),

  likes: defineTable({
    fromUserId: v.string(),
    toUserId: v.string(),
    contentType: v.union(v.literal("photo"), v.literal("prompt")),
    contentReference: v.string(),
    message: v.optional(v.string()),
  })
    .index("by_toUserId_and_fromUserId", ["toUserId", "fromUserId"])
    .index("by_fromUserId_and_toUserId", ["fromUserId", "toUserId"]),

  passes: defineTable({
    fromUserId: v.string(),
    toUserId: v.string(),
  }).index("by_fromUserId_and_toUserId", ["fromUserId", "toUserId"]),

  matches: defineTable({
    user1Id: v.string(),
    user2Id: v.string(),
    initiatingLikeId: v.id("likes"),
    active: v.boolean(),
    lastMessageAt: v.optional(v.number()),
    lastMessageSenderId: v.optional(v.string()),
    lastMessagePreview: v.optional(v.string()),
    updatedAt: v.number(),
  })
    .index("by_user1Id_and_user2Id", ["user1Id", "user2Id"])
    .index("by_user2Id_and_user1Id", ["user2Id", "user1Id"]),

  messages: defineTable({
    matchId: v.id("matches"),
    senderId: v.string(),
    content: v.string(),
    sentAt: v.number(),
    readAt: v.optional(v.number()),
  }).index("by_matchId_and_sentAt", ["matchId", "sentAt"])};
```

## Config Constants

### PROMPT_TEMPLATES

Located in `src/config/promptTemplates.ts`

```typescript
export const PROMPT_TEMPLATES = [
  { id: string, text: string, category?: string }
] as const;
```

```ts
export const createLike = mutation({
  handler: async (
    ctx,
    { fromUserId, toUserId, contentType, contentReference }
  ) => {
    // Create the like
    const likeId = await ctx.db.insert("likes", {
      fromUserId,
      toUserId,
      contentType,
      contentReference,
    });

    // Check if they liked us back
    const reciprocalLike = await ctx.db
      .query("likes")
      .withIndex("by_fromUserId_and_toUserId", (q) =>
        q.eq("fromUserId", toUserId).eq("toUserId", fromUserId)
      )
      .first();

    if (reciprocalLike) {
      // They liked us back! Check if match already exists
      const [user1Id, user2Id] = [fromUserId, toUserId].sort();

      const existingMatch = await ctx.db
        .query("matches")
        .withIndex("by_user1Id_and_user2Id", (q) =>
          q.eq("user1Id", user1Id).eq("user2Id", user2Id)
        )
        .unique();

      if (!existingMatch) {
        // Create the match
        await ctx.db.insert("matches", {
          user1Id,
          user2Id,
          initiatingLikeId: likeId,
          active: true,
          updatedAt: Date.now(),
        });
      }
    }

    return likeId;
  },
});
```
